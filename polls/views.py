...
def welcome_poll(request):
    ...
    response = render(
        request,                # HttpRequest
        "polls/welcome.html",   # template 파일의 경로
        {"now":now} # view가 template에 전달할 값들을 dictionary에 name-value로 묶어서 전달: context value
    )
    # respinse: HttpResponse(polls/welcome.html 처리 내용)
    print("=============", type(response))
    return response
...
def list_no_paging(request):
    ...
    # render(): template를 실행해서 그 결과로 HttpResponse를 반환하는 함수.
    return render(
...
# - 요청파라미터(querystring)으로 page번호를 받는다
...
def list(request):
    ...
    page_group_count = 10  # 한 페이지 그룹 당 페이지 개수
    # http://ip:port/polls/list?page=15
    current_page = int(request.GET.get('page', 1)) # 현재 조회하려는 페이지 번호
...
#   - 설문 질문 id를 받아서 그 질문에 대해 보기를 선택할 수 있는 페이지를 응답
...
# - 응답: 투표 결과를 응답. 질문 - 보기(choice_text, votes)
#         요청파라미터 검증 실패(아무 것도 선택없이 요청) - vote_form.html 이동(다시 투표하도록)
...
def vote(request):
    ...
    #########################################################################
    # 쿠키(Cookie)를 이용해서 이미 투표한 적이 있는 질문이면 투표를 못 하게 처리.
    #   - 쿠기 연습용 (실제는 DB를 통해 처리해야 한다.)
    # 1. 쿠키 voted_question에 question_id가 있는지 여부를 확인
    ...
    if voted_question_ids is not None:
        # 투표한 질문 ID를 쿠키에 "1, 2, 3, 10, 5" `,`를 구분자로 저장
    ...
    if choice_id: # 선택이 된 경우(정상처리)
        ...
        print(type(response))
        # voted_question 쿠기에 투표한 질문 ID를 셋팅
        # 처음 투표일 경우 (voted_question_ids == None)는 "question_id" 반환
        # 기존 투표한 값이 있을 경우 "1,2,3,question_id"형태로 기존 것에 추가해서 반환
        ...
        question = Question.objects.get(pk=question_id)
        return render(request, "polls/vote_form.html", {"question":question, "error_message": "골라"})
...
#   - POST방식 요청: 등록 처리
# 응답 template
#   - GET방식 요청: polls/vote_create.html
#   - POST방식 요청: list로 이동 => redirect 방식으로 이동
...
def vote_create(request):
    ...
    if http_method == "GET":
        ...
        # 같은 이름으로 여러 개 값이 전달된 경우 getlist("요청파라미터 이름") - list
